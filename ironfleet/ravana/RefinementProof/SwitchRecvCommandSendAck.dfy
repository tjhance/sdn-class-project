...
module Refinement_Proof_SwitchRecvCommandSendAck {
  ...
  predicate conditions(rs: RState, rs': RState, command_id: int)
  {
    rstate_valid(rs)
  ...
  {
    lemma_packets_are_valid(rs, rs', command_id);
    lemma_log_is_valid(rs, rs', command_id);
  ...
  {
    packet_validation_preservation(rs, rs', command_id);
    lemma_packets_are_valid_sending_1(rs, rs');
  }

  lemma packet_validation_preservation(rs: RState, rs': RState, command_id: int)
  requires conditions(rs, rs', command_id)
